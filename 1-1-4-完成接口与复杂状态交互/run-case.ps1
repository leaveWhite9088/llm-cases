...
$cli=Join-Path (Split-Path -Parent $PSScriptRoot) 'case-cli.ps1';$command=if($TestOnly){'test'}else{'run'};$arguments=@($command,'-Cases','1-1-4','-MaxBudgetUsd',$MaxBudgetUsd,'-Port',$Port)
